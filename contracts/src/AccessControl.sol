...
contract AccessControl {
    ...
    event PodeTokenUpdated(address indexed newToken);
    ...
    function isHolder(address _address) public view returns (bool) {
        return ERC721(podeToken).balanceOf(_address) >= 1;
    }
    ...
    function setPodeToken(address _podeToken) external onlyAdmin {
        podeToken = _podeToken;
        emit PodeTokenUpdated(_podeToken);
    }
}
